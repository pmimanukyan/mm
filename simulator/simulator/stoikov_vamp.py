...
from simulator import \
    MdUpdate, Order, OwnTrade, Sim, update_positions
...
class StoikovStrategyVAMP:
        ...
        while True:
            ...
            for update in updates:
                ...
                if isinstance(update, MdUpdate):
                    ...
                    self.asks, self.bids = update_positions(
                        self.asks, self.bids, update)
    ...
    def get_central_price(self):
        ...
        for i in range(len(self.asks)):
            P_a += self.asks[i][0] * (self.asks[i][1] * self.asks[i][0])
            Q_a += (self.asks[i][1] * self.asks[i][0])
        P_a /= Q_a

        for i in range(len(self.bids)):
            P_b += self.bids[i][0] * (self.bids[i][1] * self.bids[i][0])
            Q_b += (self.bids[i][1] * self.bids[i][0])
        P_b /= Q_b
        ...
        # print("P_a, P_b, vamp", P_a, P_b, vamp)
        # exit(0)
        return vamp
